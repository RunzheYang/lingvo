...
class TransformerBatchMajorEncoderMixed(base_layer.BaseLayer):
  """Transformer encoder with batch major implementation.

  Support extra shared embeddings
  ...
  def Params(cls):
    ...
    p.Define('shared_emb', None, 'Embedding shared with Decoder.')
    p.Define('shared_emb_ex', None, 'Secondary embedding shared with Decoder.')
  ...
  def __init__(self, params):
    ...
    if p.shared_emb:
      self.CreateChild('softmax', p.shared_emb)
    elif p.shared_emb_ex:
      self.CreateChild('softmax', p.shared_emb_ex)
  ...
  def _CreateChildrenVariables(self):
    if self.params.shared_emb:
      with tf.variable_scope('shared_emb', reuse=tf.AUTO_REUSE):
        self.softmax.InstantiateVariables()
    elif self.params.shared_emb_ex:
      with tf.variable_scope('shared_emb_ex', reuse=tf.AUTO_REUSE):
        self.softmax.InstantiateVariables()
    super()._CreateChildrenVariables()
